...
def main(num_steps=5000, save_path="lidar_dataset.npz"):
    """
    Generate a dataset of LiDAR scans and left-wall distances.
    ...
    fov = 4.7
    angles = np.linspace(-fov / 2, fov / 2, 1080)  # 1080 beams
    ...
    for step in range(num_steps):
        ...
        # --- LiDAR scan ---
        lidar_scan = obs["scans"]  # shape: (1080,)

        # --- Compute left-wall distance ---
        y_coords = lidar_scan * np.sin(angles)
        left_wall_dist = np.max(y_coords)

        # --- Store in dataset ---
        X.append(lidar_scan)
        y.append(left_wall_dist)
    ...
    X = np.array(X)  # shape = (num_samples, 1080)
    y = np.array(y)  # shape = (num_samples,)
    np.savez_compressed(save_path, X=X, y=y)
    print(f"Dataset saved to {save_path}, samples collected: {len(X)}")

    env.close()


if __name__ == "__main__":
    # Example: run for 5000 steps
    main(num_steps=5000, save_path="lidar_dataset.npz")
