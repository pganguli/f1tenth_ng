...
class EdgeCloudPlanner(BasePlanner):  # pylint: disable=too-many-instance-attributes
    ...
    def plan(self, obs: dict[str, Any], ego_idx: int = 0) -> Action:
        ...
        if self._latest_cloud_action is not None:
            context["deviation"] = self._action_distance(
                edge_action,
        ...
        if self._latest_cloud_features is not None:
            ...
            left = (
                self.alpha_left * c_left
                + (1.0 - self.alpha_left) * self.edge_planner.last_left_dist
            )
            track = (
                self.alpha_track * c_track
                + (1.0 - self.alpha_track) * self.edge_planner.last_track_width
            )
            heading = (
                self.alpha_heading * c_head
                + (1.0 - self.alpha_heading) * self.edge_planner.last_heading_error
            )
