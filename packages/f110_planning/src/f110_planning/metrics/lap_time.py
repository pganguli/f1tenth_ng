...
def has_collision(obs: dict[str, Any], ego_idx: int = 0) -> bool:
    """Return ``True`` if the ego vehicle has collided according to the observation.

    Args:
        obs: Observation dictionary from ``F110Env.step()``.
        ego_idx: Index of the ego vehicle.

    Returns:
        ``True`` when ``obs["collisions"][ego_idx] > 0``.
    """
    return bool(obs["collisions"][ego_idx] > 0)
