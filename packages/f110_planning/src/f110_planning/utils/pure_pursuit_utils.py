...
def nearest_point(
    ...
    Args:
        ...
        trajectory (numpy.ndarray, (N, 2)): array of (x, y) trajectory waypoints
            NOTE: points in trajectory should ideally be unique. Degenerate
            segments (consecutive identical points) are now handled gracefully
            by treating their projection parameter as zero, effectively ignoring
            the zero-length segment. This prevents numba from raising a
            ``SystemError`` on division by zero.
    ...
    # points result in a zero-length segment which would make ``l2s`` zero
    # and trigger a division-by-zero inside the njitted routine. numba turns
    # such a condition into a mysterious ``SystemError`` the caller sees, so
    # we clamp or ignore degenerate segments instead.
    diffs = trajectory[1:, :] - trajectory[:-1, :]
    l2s = diffs[:, 0] ** 2 + diffs[:, 1] ** 2

    # compute the projection factors safely, avoiding divide-by-zero
    dots = np.empty((trajectory.shape[0] - 1,))
    t = np.empty((trajectory.shape[0] - 1,))
    for i in range(dots.shape[0]):
        dots[i] = np.dot((point - trajectory[i, :]), diffs[i, :])
        if l2s[i] <= 0.0:
            # degenerate segment; treat the projection parameter as 0 so that
            # the point on this segment is simply the first waypoint. this
            # effectively ignores the segment when searching for the nearest
            # point, which is the most reasonable fallback.
            t[i] = 0.0
        else:
            t[i] = dots[i] / l2s[i]
